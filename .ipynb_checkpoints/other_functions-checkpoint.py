...
def aflow2pmg_entry(entry):
    auid = entry.attributes['auid']
    if not entry.attributes['spacegroup_relax']:
        return None
    try:
        s = adaptor.get_structure(entry.atoms())
    except HTTPError as err:
        return None

    sg = SpacegroupAnalyzer(s)
    s = sg.get_conventional_standard_structure()
    e = len(s) * entry.attributes['energy_atom']
